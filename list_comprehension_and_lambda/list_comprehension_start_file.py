...
newlist = [format(round(x,2)) for x in numbers if x > 0 ]
print(newlist)
...
list1 = [len(x) for x in words if x[0] != 't']
print(list1)
...
list1 = [x.upper() for x in dict if dict[x] < 5000]
print(list1)



## Find all the numbers from 1 to 1000 that have a 4 in them
numbers = [x for x in range(0,1000) if '4' in str(x)]
print(numbers)
...
list1 = len([x for x in txt if x.lower == 'the'])
print(list1)
